# Sort timelapses in seperate folders by difference in timestamp
# Todo: Port to Python 3 so you can just compile it as a package!
# Todo: works for one card folder only at the moment, make a different output folder per card.
# Folder structure is: Root project folder - Day 1, Day 2 - etc... - Either sub folder in day 1 with cards or direct shots in day 1
# CR2 RAW Files.
# Note: Each memory card gets its own folder (also when shooting 2 at the same time)

# Todo: Scan the root folder for folders and traverse them seperately
# Todo: check for sub folders (memory cards) in the folder. If subfolder process sub folder, otherwise
# Todo: Call the process function per folder. (Assumption: timelapse is always whole in one folder)
